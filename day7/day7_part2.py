...
class FileSystem:
    ...
    def getAllDirectories(self):
        return [self.fs] + self.getChildDirectories(self.fs)
...
required_filesystem_size = 30000000
...
used_filesystem_size = next(int(elem['folderSize']) for elem in test.getAllDirectorySizes() if elem['folderName'] == '/')
remaining_filesystem_size = total_filesystem_size - used_filesystem_size
space_still_needed = required_filesystem_size - remaining_filesystem_size

folderSizes = test.getAllDirectorySizes()
folderSizes.sort(key=(lambda e: int(e['folderSize'])))

answer = 0
for folder in folderSizes:
    if folder['folderSize'] > space_still_needed:
        answer = folder['folderSize']
        break
