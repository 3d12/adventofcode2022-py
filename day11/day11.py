...
class Monkey:
    def __init__(self, data):
        ...
        self.operation = (lambda e: (e if term1 == 0 else term1) + (e if term2 == 0 else term2)) if self.operation_str.split()[1] == '+' else (lambda e: (e if term1 == 0 else term1) * (e if term2 == 0 else term2))
...
for i in range(20):
    ...
    for j in range(len(monkeys)):
        print(f"Monkey {j}: {monkeys[j].held_items}")
    for monkey in monkeys:
        thrown_items = monkey.monkeyTurn()
