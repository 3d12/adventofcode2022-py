...
shortest_hiking_trail = 0
for i in range(len(possible_starts)):
    print(f"DEBUG: Trying start {i+1} of {len(possible_starts)}")
    possible_start = possible_starts[i]
    length_from_start = shortest_path(input_list, possible_start[0], possible_start[1])
    if shortest_hiking_trail == 0 or length_from_start < shortest_hiking_trail:
        shortest_hiking_trail = length_from_start

print(f"The answer is {shortest_hiking_trail}")
