...
class SandGrid:
        ...
        for line in self.lines:
            for coordPair in line:
                for x,y in coordPair:
                    if x < self.min_x:
                        self.min_x = x
                    if x > self.display_max_x:
                        self.display_max_x = x
                    if y < self.min_y:
                        self.min_y = y
                    if y > self.max_y:
                        self.max_y = y
        self.floor_level = self.max_y+2
        floor_line = [(0, self.floor_level), (self.max_x, self.floor_level)]
        self.filled_lines.append([self.lineFill(floor_line)])
        self.grid = []
        for y in range(self.floor_level+1):
    ...
    def __str__(self):
        ...
        for displayLine in self.grid_w_structs:
            displayRegion = displayLine[self.min_x-1:self.display_max_x]
    ...
    def dropSand(self):
        ...
        while not self.overflow:
            ...
            while not self.stable:
                #print(f"DEBUG: self.sandPath={self.sandPath}")
                current_pos = self.sandPath[-1]
                #print(f"DEBUG: current_pos={current_pos}, current_pos == (500,0) = {current_pos == (500,0)}")
